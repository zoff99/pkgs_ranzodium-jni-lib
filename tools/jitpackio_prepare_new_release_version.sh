...
cur_m_version=$(echo "$f3" | sed -e 's#^.*local_maven_ranzodium_jni_##'|sed -e 's#.zip$##')
...
sed -i -e 's#unzip local_maven_ranzodium_jni_.*$#unzip local_maven_ranzodium_jni_'"$next_m_version"'.zip#' "$f1"
